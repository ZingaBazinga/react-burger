...
export function BurgerConstructor() {
  ...
  const bunIbgredient = constructorItems.find(
  ...
  const [, drop] = useDrop({
    ...
    drop: (item: IIngredient) => {
      if (
        constructorItems.find(
          (ingredient) => ingredient._id === item._id && item.type === "bun",
        )
      ) {
        return;
      } else if (
        ...
        const oldBun = burgerIngredients.find(
          (item) => item.__v !== 0 && item.type === "bun",
        );
    ...
    <div ref={combinedRef} className={`${styles.container}`}>
      ...
      <div className={`${styles.place_an_order}`}>
        ...
        <Button
          disabled={
            totalPrice === 0 ||
            burgerConstructor.bun === undefined ||
